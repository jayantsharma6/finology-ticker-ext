...
const DataService = (function () {
  ...
  const _DUMMY_COMPANY_LIST = [
    ...
    { ticker: 'SUNPHARMA',  name: 'Sun Pharmaceutical Ltd'        },
    { ticker: 'ONGC',       name: 'Oil & Natural Gas Corp Ltd'    },
    { ticker: 'NTPC',       name: 'NTPC Ltd'                      },
    { ticker: 'POWERGRID',  name: 'Power Grid Corp of India Ltd'  },
  ...
  const _DUMMY_STOCK_DETAIL = {
    RELIANCE:   { ticker: 'RELIANCE',   name: 'Reliance Industries Ltd',       price: 2985.40, changePercent: 1.45,   changeAbsolute: 42.60,   sector: 'Energy',     exchange: 'NSE' },
    TCS:        { ticker: 'TCS',        name: 'Tata Consultancy Services Ltd',  price: 4120.15, changePercent: -0.82,  changeAbsolute: -34.20,  sector: 'IT',         exchange: 'NSE' },
    HDFCBANK:   { ticker: 'HDFCBANK',   name: 'HDFC Bank Ltd',                 price: 1445.20, changePercent: 0.35,   changeAbsolute: 5.05,    sector: 'Banking',    exchange: 'NSE' },
    INFY:       { ticker: 'INFY',       name: 'Infosys Ltd',                   price: 1612.45, changePercent: 2.10,   changeAbsolute: 33.15,   sector: 'IT',         exchange: 'NSE' },
    ICICIBANK:  { ticker: 'ICICIBANK',  name: 'ICICI Bank Ltd',                price: 1089.30, changePercent: -1.20,  changeAbsolute: -13.25,  sector: 'Banking',    exchange: 'NSE' },
    HINDUNILVR: { ticker: 'HINDUNILVR', name: 'Hindustan Unilever Ltd',        price: 2345.60, changePercent: 0.75,   changeAbsolute: 17.40,   sector: 'FMCG',       exchange: 'NSE' },
    SBIN:       { ticker: 'SBIN',       name: 'State Bank of India',           price: 772.15,  changePercent: 0.95,   changeAbsolute: 7.30,    sector: 'Banking',    exchange: 'NSE' },
    BAJFINANCE: { ticker: 'BAJFINANCE', name: 'Bajaj Finance Ltd',             price: 6842.50, changePercent: -0.60,  changeAbsolute: -41.35,  sector: 'Finance',    exchange: 'NSE' },
    TATAMOTORS: { ticker: 'TATAMOTORS', name: 'Tata Motors Ltd',               price: 965.80,  changePercent: 3.45,   changeAbsolute: 32.25,   sector: 'Automobile', exchange: 'NSE' },
    ITC:        { ticker: 'ITC',        name: 'ITC Ltd',                       price: 428.90,  changePercent: 0.0,    changeAbsolute: 0.0,     sector: 'FMCG',       exchange: 'NSE' },
    AXISBANK:   { ticker: 'AXISBANK',   name: 'Axis Bank Ltd',                 price: 1145.60, changePercent: -0.45,  changeAbsolute: -5.20,   sector: 'Banking',    exchange: 'NSE' },
    KOTAKBANK:  { ticker: 'KOTAKBANK',  name: 'Kotak Mahindra Bank Ltd',       price: 1789.30, changePercent: 1.10,   changeAbsolute: 19.50,   sector: 'Banking',    exchange: 'NSE' },
    WIPRO:      { ticker: 'WIPRO',      name: 'Wipro Ltd',                     price: 456.75,  changePercent: -0.30,  changeAbsolute: -1.40,   sector: 'IT',         exchange: 'NSE' },
    HCLTECH:    { ticker: 'HCLTECH',    name: 'HCL Technologies Ltd',          price: 1523.40, changePercent: 1.85,   changeAbsolute: 27.65,   sector: 'IT',         exchange: 'NSE' },
    MARUTI:     { ticker: 'MARUTI',     name: 'Maruti Suzuki India Ltd',       price: 11245.00,changePercent: 0.55,   changeAbsolute: 61.50,   sector: 'Automobile', exchange: 'NSE' },
    SUNPHARMA:  { ticker: 'SUNPHARMA',  name: 'Sun Pharmaceutical Ltd',        price: 1456.20, changePercent: -0.90,  changeAbsolute: -13.20,  sector: 'Pharma',     exchange: 'NSE' },
    ONGC:       { ticker: 'ONGC',       name: 'Oil & Natural Gas Corp Ltd',    price: 267.45,  changePercent: 2.30,   changeAbsolute: 6.00,    sector: 'Energy',     exchange: 'NSE' },
    NTPC:       { ticker: 'NTPC',       name: 'NTPC Ltd',                      price: 345.60,  changePercent: 0.65,   changeAbsolute: 2.25,    sector: 'Power',      exchange: 'NSE' },
    POWERGRID:  { ticker: 'POWERGRID',  name: 'Power Grid Corp of India Ltd',  price: 289.75,  changePercent: -0.20,  changeAbsolute: -0.60,   sector: 'Power',      exchange: 'NSE' },
    ULTRACEMCO: { ticker: 'ULTRACEMCO', name: 'UltraTech Cement Ltd',          price: 9876.50, changePercent: 1.20,   changeAbsolute: 117.00,  sector: 'Cement',     exchange: 'NSE' },
  };
  ...
  async function _fetchStockDetail(ticker) {
    // FUTURE: const res = await fetch(API_ENDPOINTS.stockDetail + '/' + ticker);
    // FUTURE: return await res.json();
    return _DUMMY_STOCK_DETAIL[ticker] || null;
  }
  ...
  async function _loadStockDetail(ticker) {
    const upper = ticker.toUpperCase();

    if (_mem.stockDetail[upper]) {
      const fetchMap  = await StorageService.get(STORAGE_KEYS.STOCK_DETAIL_FETCH) || {};
      const lastFetch = fetchMap[upper] || null;
      if (!CachePolicy.shouldRefreshMarketData(lastFetch)) {
        return _mem.stockDetail[upper];
      }
    }
    ...
    if (!CachePolicy.shouldRefreshMarketData(lastFetch) && storedMap[upper]) {
      _mem.stockDetail[upper] = storedMap[upper];
      return storedMap[upper];
    }

    const data = await _fetchStockDetail(upper);
    if (!data) return null;

    _mem.stockDetail[upper]  = data;
    storedMap[upper]         = data;
    fetchMap[upper]          = Date.now();
  ...
  async function getStockDetail(ticker) {
    return _loadStockDetail(ticker);
  }
